...
Decidim.stats.register :Participantes,
  ...
  unique_user_ids = chatbot_users.where.not(decidim_user_id: nil).distinct.pluck(:decidim_user_id)
...
                      .count
  ...
  (unique_user_ids + website_user_ids).uniq.count + STATIC_ADDITIONAL_USERS + anonymous_count
end
